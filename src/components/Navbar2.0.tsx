...
const navItems: NavItem[] = [
  {
    icon: "/assets/home/home-outline.svg",
    label: "Home",
    link: "/",
    rotation: "",
    tooltipStyle: { left: "-78px" },
  },
  {
...
const Navbar: React.FC = () => {
  ...
  return (
    <div
      className={`fixed bottom-[300px] right-[570px] ${
        navbarActive ? "active" : ""
      }`}
      id="mydiv"
    >
      ...
      >
              ...
              className={`h-[5px] w-[26px] rounded-[20px] bg-[#d3531a] opacity-100 transition-all duration-250 ease-in-out ${
                navbarActive
                  ? index === 1
                    ? "opacity-0"
                    : "absolute top-[18px] " +
                      (index === 0 ? "rotate-[135deg]" : "-rotate-[135deg]")
                  : ""
              }`}
            />
          ))}
        ...
        <ul className="relative">
          {navItems.map((item, index) => {
            const angle = -(Math.PI / 8) * index - Math.PI / 2 + 0.05; // Arch effect
